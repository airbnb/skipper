...
// skipper-metrics-prometheus: a `Metrics` implementation for one observability backend. Lives under plugins/ and is
...
dependencies {
    ...
    api(project(":"))
    // The Prometheus client is `api`: adopters pass in (and scrape) the `PrometheusRegistry` that
    // `PrometheusMetrics` registers its collectors with.
...
// Publishing
// ---------------------------------------------------------------------------------------
// Published to Maven Central as com.airbnb.skipper:skipper-metrics-prometheus by the same `publishToMavenCentral`
...
mavenPublishing {
    coordinates("com.airbnb.skipper", "skipper-metrics-prometheus", version.toString())
    configure(KotlinJvm(javadocJar = JavadocJar.Dokka("dokkaJavadoc"), sourcesJar = true))
    pom {
        name.set("Skipper Metrics for Prometheus")
        description.set("Skipper Metrics implementation that registers collectors with the Prometheus Java client.")
